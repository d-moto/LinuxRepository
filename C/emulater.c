...
//命令コードのインデックス
//機械語の上位4bit(0000(0)から1111(15))まで
#define MOV 0
#define ADD 1
#define SUB 2
#define AND 3
...
//インストラクションレジスタ（命令レジスタ）から命令コードやオペランドを抽出する関数群のプロトタイプ宣言
short op_code(short);
short op_regA(short);
short op_regB(short);
short op_data(short);
...
void main(void) {  //mainの開始
	// 定数の定義
	short pc; //プログラムカウンタ
	short ir; //インストラクションレジスタ
	short flag_eq; //比較用フラグ

	assembler();//簡易アセンブラの関数呼び出し
	...
	do {
		...
		ir = rom[pc]; //pc番地の機械語をインストラクションレジスタに転送
		printf("%5d %5x %5d %5d %5d %5d \n", pc, ir, reg[0], reg[1], reg[2], reg[3]);

		pc = pc + 1; //プログラムカウンタの更新
		...
		case SR: reg[op_regA(ir)] = reg[op_regB(ir)] >> 1;
			break;
